...
export async function POST(request) {
  try {
    ...
    const brancheLabel = benchmarkData[branche] ? benchmarkData[branche].branche : branche
    await sendNotificationToOwner({
      subject: 'Benchmark-Report Bestellung: ' + company + ' - ' + brancheLabel,
      html: '<h2>Neue Benchmark-Report Bestellung</h2><table style="border-collapse:collapse;width:100%;max-width:500px;"><tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;">Name</td><td style="padding:8px;border:1px solid #ddd;">' + name + '</td></tr><tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;">Firma</td><td style="padding:8px;border:1px solid #ddd;">' + company + '</td></tr><tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;">E-Mail</td><td style="padding:8px;border:1px solid #ddd;">' + email + '</td></tr><tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;">Branche</td><td style="padding:8px;border:1px solid #ddd;">' + brancheLabel + '</td></tr></table>',
    })
    await sendConfirmationToCustomer({
      to: email,
      subject: 'Ihre Benchmark-Report Anfrage - ' + brancheLabel,
      html: '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;"><div style="background:linear-gradient(135deg,#1e3a8a,#2563eb);padding:30px;text-align:center;border-radius:12px 12px 0 0;"><h1 style="color:white;margin:0;font-size:24px;">Benchmark-Report Anfrage</h1></div><div style="padding:30px;background:white;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;"><p>Hallo ' + name + ',</p><p>vielen Dank fuer Ihre Bestellung des <strong>Branchen-Benchmark Reports</strong> fuer <strong>' + brancheLabel + '</strong>.</p><p>Ihr individueller Report wird innerhalb von <strong>3-5 Werktagen</strong> erstellt.</p><p>Mit freundlichen Gruessen,<br><strong>Steffen Hefter</strong><br>KI-Berater | frimalo</p></div></div>',
    })
