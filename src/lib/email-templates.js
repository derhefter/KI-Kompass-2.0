...
export function getCustomerEmailForPlan({ plan, safeName, safeCompany, amount, accessCode, accessLink, expiresAtFormatted, planName }) {
  switch (plan) {
    ...
    case 'toolbox-pro':
      return getToolboxEmail({ safeName, safeCompany, amount, planName, isPro: plan === 'toolbox-pro', accessCode, accessLink, expiresAtFormatted })
...
// ============================================================
function getToolboxEmail({ safeName, safeCompany, amount, planName, isPro, accessCode, accessLink, expiresAtFormatted }) {
  ...
  return {
    ...
    html: wrapEmail({
      ...
      bodyHtml: `<h2 style="color:#1e3a8a;margin-top:0;">Herzlich willkommen, ${safeName}!</h2><p>Ihre Zahlung &uuml;ber <strong>${amount} &euro;/Monat</strong> f&uuml;r die <strong>${planName}</strong> wurde erfolgreich verarbeitet.</p>${accessCodeBlock(accessCode, accessLink, expiresAtFormatted, 'Jetzt Toolbox-Bereich &ouml;ffnen')}<h3 style="color:#1e3a8a;">In Ihrer ${isPro ? 'Professional' : 'Starter'} Membership enthalten:</h3><ul>${resourceHtml}</ul><div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:16px;margin:16px 0;"><p style="margin:0;font-size:14px;color:#1e40af;"><strong>N&auml;chster Schritt:</strong> Klicken Sie auf den Button oben, um Ihren Toolbox-Bereich zu &ouml;ffnen. Dort absolvieren Sie zuerst ein kurzes Assessment und erhalten dann Zugang zu allen Ressourcen.</p></div>`,
    }),
  }
}
